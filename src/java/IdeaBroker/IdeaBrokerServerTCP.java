...
import static IdeaBroker.IdeaBrokerServerTCP.notificacoes;
import java.io.*;
import java.net.*;
import java.util.*;
import java.rmi.*;
...
public class IdeaBrokerServerTCP  extends UnicastRemoteObject implements ClienteRMIInterface
{
  ...
  /*public static Vector<String> onlineUsers = new Vector<String>();
    ...
    */
       ...
       /* System.getProperties().put("java.security.policy", "policy.all");
        ...
        try {
               ...
               try{
                    ...
                    System.out.println("//"+ IP_rmi +":1099/Ideia");
                    serverRMI = (IdeiaMethods) Naming.lookup("//"+ IP_rmi +":1099/Ideia");
                
                } catch(RemoteException e) {

                     serverRMI = (IdeiaMethods) Naming.lookup("//"+ IP_rmi +":6000/Ideia");
                
                }
               
               //cliente = new IdeaBrokerServerTCP();
            /*   try{
                    serverRMI.AdicionaCLiente((ClienteRMIInterface) cliente);
               } catch(RemoteException e){
                    System.out.println("Errp AdicionarCliente..");
                    return;
	       }*/
        } catch (Exception e) {

                System.out.println("Exception in main: RMI == "+ IP_rmi +"\n");
                return;
        ...
        while (true) {
            ...
            if (type.compareTo("P") == 0) {
                ...
                novoCliente.start();
                System.out.println("Vou ligar ao RMI!!!!!!!!!!");
                cliente = new IdeaBrokerServerTCP();
                serverRMI.AdicionaCLiente((ClienteRMIInterface) cliente);
    ...
    @Override
    public void addNotification(String username, String notificacao) throws RemoteException 
    {       
        System.out.println("Entra no addNotification == "+listaThreads.size());
        if(!listaThreads.isEmpty())
        for(ClienteTCP _cliente: listaThreads)
        {
            System.out.println("Cliente == "+_cliente.getUsername());
            
            if(_cliente.getUsername().compareTo(username) == 0)
            {
                _cliente.enviar_notificacao(username, notificacao);
            }
        }
    }
...
class ClienteTCP extends Thread 
{
	...
	public ClienteTCP(Socket _cliente_informacao, Socket _cliente_estado) throws RemoteException
	{
    ...
    public void addNotification(String username, String notificacao) throws RemoteException
    {
         System.out.println("[thread] Estou a enviar uma notificação utilizador == "+username+"\n");
        // System.out.println("E a notificacao == "+notas+"\n");
        /** Envia notificacao para o socket aberto **/
        outputMensagem.println("qwerty");
        outputMensagem.println(username);
        outputMensagem.println(notificacao);
       // notificacoes.add(note);
    }
    
    public void enviar_notificacao(String utilizador,String notas)
    {
        System.out.println("[thread] Estou a enviar uma notificação utilizador == "+utilizador+"\n");
       // System.out.println("E a notificacao == "+notas+"\n");
        /** Envia notificacao para o socket aberto **/
        outputMensagem.println("qwerty");
          ...
          /*  if(Notificacoes != null)
            ...
            switch (escolha) {
                ...
                case 163:{
                    ...
                    // Utilizador saiu
                    try{
                       //h.retiraOnline((ClienteRMIInterface)cliente);
                        //h.removerUsersOnline(eu.getusername());
                        System.out.println("Utilizador saiu\n");
                       // h.RemoverLigacao((ClienteRMIInterface) cliente);
                    } catch (Exception e) {
    ...
    public void bolsa()
    {
        ...
        int investimento=inputControlo.nextInt();
        System.out.println("Recebi investimento=" + investimento);
        int novopreco=inputControlo.nextInt();
        System.out.println("Recebi Novo Preço=" + novopreco);
        ...
        //Retornar String com todos os Utilizadores que detem alguma share na ideia selecionada para o utilizador comprar
            try {	
                ...
                //NOVA VERSAO BOLSA de VALORES
                int controlo=h.efectuarcompra(eu.getusername(),idarraytopico, idarrayideia, percentagem, novopreco,investimento);
                ...
                if(controlo == 1)
                {
                   ...
                   /* try {
                       // System.out.println("Vou enviar uma notificacao ao RMI...\n");
                        ...
                        h.adicionarNotasAgendadas("Compraram as tuas acções.",utilizador);
        ...
        switch (escolha) {
            ...
            case 1:
                ...
                try {
                    ...
                    controlo=h.criarideia(id_topico, texto, eu.getusername(),null,100);
            ...
            case 2:
    			...
    			try {
    				...
    				controlo=h.criarideia(id_topico,"",eu.getusername(),_anexo,100);
            ...
            case 3:
                ...
                try {
                    ...
                    controlo=h.criarideia(id_topico,texto,eu.getusername(),_anexo,100);
    ...
    public synchronized void criartopico()
    {
        ...
        try {
            ...
            h.adicionartopico(tema,eu.getusername());
        ...
        try{
            ...
            switch(escolha)
            {
                case 1: {
                    ...
                    {
                        //h.AdicionaCLiente(cliente);
                        eu = dados_utilizador;
                        meu_utilizador = dados_utilizador.getusername();
                        outputControlo.println(true);
